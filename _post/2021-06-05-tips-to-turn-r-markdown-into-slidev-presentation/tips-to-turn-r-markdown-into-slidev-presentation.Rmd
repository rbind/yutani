...
preview: ./images/slide.jpeg
creative_commons: CC BY
---


[Slidev](https://sli.dev) is a tool to create slides from Markdown. Recently,
I used this with R Markdown and it was generally comfortable to use.
...
(You might wonder what this presentation talks about R and Rust, but the contents
...
These slides are generated from `slides.md` by a local Node.js server.
`slides.md` is located in the top directory of the project created right now.
...
directory so that we can render it to overwrite `slides.md`. The Node.js server
detects the change on `slides.md` and regenerates slides from it on the fly.

We can stop the server with Ctrl+C on console.
To launch again, execute this:
...
First of all, the target we want to generate is a Markdown file, so let's specify
`md_document` as the `output`.
...
What's a bit tricky here is that Slidev also uses YAML front-matter for defining
metadata. So, we need to put both items for R Markdown and those for Slidev
together, and preserve it after rendering. Let's specify `preserve_yaml: true`.
...
Then, put settings for Slidev.
There might be some name collision between R Markdown and Slidev, but I don't find it yet, fortunately.
The full YAML would be like this:
...
specify `layout` per slide like this
(`image` is a specific parameter for `image-right` layout):
...
But, for unknown reason, this won't work (I don't find the reason yet, but I think
something is happening in conversions by Pandoc):
...
Slidev needs Node.js server to serve the slides, but it can also be exported as an
standalone single-page application by the following command:
...
or xaringan, as Slidev requires Node.js to build or run, which is not very portable.
So, I'm not sure how useful this post is to R users, but hope you enjoy!
