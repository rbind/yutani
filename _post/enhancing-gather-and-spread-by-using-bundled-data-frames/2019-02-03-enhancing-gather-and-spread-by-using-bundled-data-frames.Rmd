...
```{r echo=FALSE}
gt(d_gathered) %>%
  tab_style(cell_text(color = text_colours[1], weight = "bold"), cells_body(columns = vars(key), rows = 1:3)) %>% 
  tab_style(cell_text(color = text_colours[2], weight = "bold"), cells_body(columns = vars(key), rows = 4:6))
```
