...
gt(d) %>%
  ...
  cols_move(vars(Female_LoTR, Female_TT, Female_RoTK), after = vars(Race)) %>%
  tab_spanner("Female", vars(Female_LoTR, Female_TT, Female_RoTK)) %>% 
  tab_style(cells_styles(text_color = text_colours[1], text_weight = "bold"), cells_column_labels(groups = Female)) %>% 
  tab_spanner("Male", vars(Male_LoTR, Male_TT, Male_RoTK)) %>% 
  tab_style(cells_styles(text_color = text_colours[2], text_weight = "bold"), cells_column_labels(groups = Male))
```
...
```{r echo=FALSE}
gt(d_gathered) %>%
  tab_style(cells_styles(text_color = text_colours[1], text_weight = "bold"), cells_data(columns  = vars(key), rows = 1:3)) %>% 
  tab_style(cells_styles(text_color = text_colours[2], text_weight = "bold"), cells_data(columns  = vars(key), rows = 4:6))
```
